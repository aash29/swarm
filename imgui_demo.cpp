...
void ImGui::ShowTestWindow(bool *p_opened) {
        ...
        if (ImGui::TreeNode("Selectables")) {
            if (ImGui::TreeNode("Basic")) {
                ...
                if (ImGui::Selectable("5. I am double clickable", selected[3],
                                      ImGuiSelectableFlags_AllowDoubleClick)) if (ImGui::IsMouseDoubleClicked(0))
    ...
    if (ImGui::CollapsingHeader("Layout")) {
        ...
        if (ImGui::TreeNode("Text Baseline Alignment")) {
            ...
            bool tree_opened = ImGui::TreeNode(
...
static void ShowExampleAppCustomRendering(bool *opened) {
    ...
    ImGui::Separator();
    {
        ...
        draw_list->PushClipRect(ImVec4(canvas_pos.x, canvas_pos.y, canvas_pos.x + canvas_size.x, canvas_pos.y +
        ...
        for (int i = 0; i < points.Size - 1; i += 2)
            draw_list->AddLine(ImVec2(canvas_pos.x + points[i].x, canvas_pos.y + points[i].y),
                               ImVec2(canvas_pos.x + points[i + 1].x, canvas_pos.y + points[i + 1].y), 0xFF00FFFF,
                               2.0f);
        draw_list->PopClipRect();
        if (adding_preview)
            points.pop_back();
    ...
    void Draw(const char *title, bool *opened) {
        ...
        for (int i = 0; i < Items.Size; i++) {
            ...
            if (!filter.PassFilter(item))
                continue;
            ImVec4 col = ImColor(255, 255,
                                 255); // A better implementation may store a type per-item. For the sample let's just parse the text.
        ...
        switch (data->EventFlag) {
            ...
            case ImGuiInputTextFlags_CallbackHistory: {
                ...
                if (data->EventKey == ImGuiKey_UpArrow) {
                    ...
                    if (HistoryPos != -1) if (++HistoryPos >= History.Size)
                        HistoryPos = -1;
                }
                ...
                if (prev_history_pos != HistoryPos) {
                    data->CursorPos = data->SelectionStart = data->SelectionEnd = data->BufTextLen = (int) snprintf(
                            data->Buf, data->BufSize, "%s", (HistoryPos >= 0) ? History[HistoryPos] : "");
                    data->BufDirty = true;
                }
            }
        }
