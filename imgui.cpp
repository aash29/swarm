...
bool ImGui::ItemAdd(const ImRect &bb, const ImGuiID *id) {
    ...
    if (IsMouseHoveringRect(bb.Min, bb.Max)) {
        ...
        if (g.HoveredRootWindow == window->RootWindow) if (g.ActiveId == 0 || (id && g.ActiveId == *id) ||
                                                           g.ActiveIdAllowOverlap ||
                                                           (g.ActiveId == window->MoveID)) if (IsWindowContentHoverable(
                window))
...
bool ImGui::IsClippedEx(const ImRect &bb, const ImGuiID *id, bool clip_even_when_logged) {
    ...
    if (!bb.Overlaps(window->ClipRect)) {
        if (!id || *id != GImGui->ActiveId) if (clip_even_when_logged || !g.LogEnabled)
            return true;
    }
    return false;
}
...
bool ImGui::IsHovered(const ImRect &bb, ImGuiID id, bool flatten_childs) {
    ...
    if (g.HoveredId == 0 || g.HoveredId == id || g.HoveredIdAllowOverlap) {
        ImGuiWindow *window = GetCurrentWindowRead();
        if (g.HoveredWindow == window || (flatten_childs && g.HoveredRootWindow == window->RootWindow)) if (
                (g.ActiveId == 0 || g.ActiveId == id || g.ActiveIdAllowOverlap) &&
                ImGui::IsMouseHoveringRect(bb.Min, bb.Max)) if (IsWindowContentHoverable(g.HoveredRootWindow))
            return true;
    }
    return false;
}
...
bool ImGui::FocusableItemRegister(ImGuiWindow *window, bool is_active, bool tab_stop) {
    ...
    if (tab_stop && window->FocusIdxAllRequestNext == IM_INT_MAX && window->FocusIdxTabRequestNext == IM_INT_MAX &&
    ...
    if (allow_keyboard_focus) if (window->FocusIdxTabCounter == window->FocusIdxTabRequestCurrent)
        return true;
...
void ImGui::NewFrame() {
    ...
    for (int i = 0; i < IM_ARRAYSIZE(g.IO.MouseDown); i++) {
        ...
        if (g.IO.MouseDown[i]) if (mouse_earliest_button_down == -1 ||
...
static void AddDrawListToRenderList(ImVector<ImDrawList *> &out_render_list, ImDrawList *draw_list) {
    ...
    IM_ASSERT((int) draw_list->_VtxCurrentIdx ==
              draw_list->VtxBuffer.Size);                   // Sanity check. Bug or mismatch between PrimReserve() calls and incrementing _VtxCurrentIdx, _VtxWritePtr etc.
...
void ImGui::RenderTextClipped(const ImVec2 &pos_min, const ImVec2 &pos_max, const char *text, const char *text_end,
    ...
    ImVec2 pos = pos_min;
    const ImVec2 text_size = text_size_if_known ? *text_size_if_known : ImGui::CalcTextSize(text, text_display_end,
                                                                                            false, 0.0f);
...
ImVec2 ImGui::GetMouseDragDelta(int button, float lock_threshold) {
    ...
    if (lock_threshold < 0.0f)
        lock_threshold = g.IO.MouseDragThreshold;
    if (g.IO.MouseDown[button]) if (g.IO.MouseDragMaxDistanceSqr[button] >= lock_threshold * lock_threshold)
        return g.IO.MousePos -
...
static void CloseInactivePopups() {
    ...
    if (n <
        g.OpenedPopupStack.Size)   // This test is not required but it allows to set a useful breakpoint on the line below
        g.OpenedPopupStack.resize(n);
}

static ImGuiWindow *GetFrontMostModalRootWindow() {
    ImGuiState &g = *GImGui;
    if (!g.OpenedPopupStack.empty()) if (ImGuiWindow *front_most_popup = g.OpenedPopupStack.back().Window) if (
            front_most_popup->Flags & ImGuiWindowFlags_Modal)
...
bool ImGui::BeginPopupModal(const char *name, bool *p_opened, ImGuiWindowFlags extra_flags) {
    ...
    if (!opened ||
        (p_opened && !*p_opened)) // Opened can be 'false' when the popup is completely clipped (e.g. zero size display)
    {
        ImGui::EndPopup();
        if (opened)
            ClosePopup(id);
...
bool ImGui::BeginPopupContextWindow(bool also_over_items, const char *str_id, int mouse_button) {
    ...
    if (ImGui::IsMouseHoveringWindow() && ImGui::IsMouseClicked(mouse_button)) if (also_over_items ||
...
bool ImGui::Begin(const char *name, bool *p_opened, const ImVec2 &size_on_first_use, float bg_alpha,
    ...
    if (first_begin_of_the_frame) {
        ...
        if (!window_was_active) {
            ...
            if (!(flags & ImGuiWindowFlags_NoFocusOnAppearing)) if (
        ...
        window->FocusIdxAllRequestCurrent = (window->FocusIdxAllRequestNext == IM_INT_MAX ||
                                             window->FocusIdxAllCounter == -1) ? IM_INT_MAX :
        ...
        window->FocusIdxTabRequestCurrent = (window->FocusIdxTabRequestNext == IM_INT_MAX ||
                                             window->FocusIdxTabCounter == -1) ? IM_INT_MAX :
        ...
        if (!(flags & ImGuiWindowFlags_NoTitleBar)) {
            ...
            ImVec2 clip_max = ImVec2(
                    window->Pos.x + window->Size.x - (p_opened ? title_bar_rect.GetHeight() - 3 : style.FramePadding.x),
    ...
    if (flags & ImGuiWindowFlags_ChildWindow) {
        ...
        if (!(flags & ImGuiWindowFlags_AlwaysAutoResize) && window->AutoFitFramesX <= 0 && window->AutoFitFramesY <= 0)
            window->Collapsed |= (window->ClippedWindowRect.Min.x >= window->ClippedWindowRect.Max.x ||
...
static void Scrollbar(ImGuiWindow *window, bool horizontal) {
    ...
    if (!horizontal)
        bb.Min.y += window->TitleBarHeight() +
                    ((window->Flags & ImGuiWindowFlags_MenuBar) ? window->MenuBarHeight() - border_size : 0.0f);
...
ImVec2 ImGui::GetContentRegionMax() {
    ImGuiWindow *window = GetCurrentWindowRead();
    ImVec2 content_region_size = ImVec2(
            window->SizeContentsExplicit.x ? window->SizeContentsExplicit.x : window->Size.x - window->ScrollbarSizes.x,
...
ImVec2 ImGui::GetWindowContentRegionMin() {
    ImGuiWindow *window = GetCurrentWindowRead();
    return ImVec2(-window->Scroll.x, -window->Scroll.y + window->TitleBarHeight() + window->MenuBarHeight()) +
...
static inline bool IsWindowContentHoverable(ImGuiWindow *window) {
    ...
    ImGuiState &g = *GImGui;
    if (ImGuiWindow *focused_window = g.FocusedWindow) if (ImGuiWindow *focused_root_window = focused_window->RootWindow) if (
            (focused_root_window->Flags & ImGuiWindowFlags_Popup) != 0 && focused_root_window->WasActive &&
...
static bool CloseWindowButton(bool *p_opened) {
    ...
    if (hovered) {
        window->DrawList->AddLine(center + ImVec2(+cross_extent, +cross_extent),
                                  center + ImVec2(-cross_extent, -cross_extent), ImGui::GetColorU32(ImGuiCol_Text));
        window->DrawList->AddLine(center + ImVec2(+cross_extent, -cross_extent),
                                  center + ImVec2(-cross_extent, +cross_extent), ImGui::GetColorU32(ImGuiCol_Text));
    }
...
bool ImGui::CollapsingHeader(const char *label, const char *str_id, bool display_frame, bool default_open) {
    ...
    if (label == NULL)
        label = str_id;
    const bool label_hide_text_after_double_hash = (label ==
                                                    str_id); // Only search and hide text after ## if we have passed label and ID separately, otherwise allow "##" within format string.
    const ImGuiID id = window->GetID(str_id);
    ...
    const float frame_height = ImMax(ImMin(window->DC.CurrentLineHeight, g.FontSize + g.Style.FramePadding.y * 2),
    ...
    bool opened = TreeNodeBehaviorIsOpened(id, (default_open ? ImGuiTreeNodeFlags_DefaultOpen : 0) |
...
void ImGui::Bullet() {
    ...
    window->DrawList->AddCircleFilled(bb.Min + ImVec2(style.FramePadding.x + g.FontSize * 0.5f, line_height * 0.5f),
...
bool ImGui::DragFloatRange2(const char *label, float *v_current_min, float *v_current_max, float v_speed, float v_min,
    ...
    bool value_changed = ImGui::DragFloat("##min", v_current_min, v_speed, (v_min >= v_max) ? -FLT_MAX : v_min,
                                          (v_min >= v_max) ? *v_current_max : ImMin(v_max, *v_current_max),
                                          display_format, power);
    ImGui::PopItemWidth();
    ImGui::SameLine(0, g.Style.ItemInnerSpacing.x);
    value_changed |= ImGui::DragFloat("##max", v_current_max, v_speed,
...
bool ImGui::DragIntRange2(const char *label, int *v_current_min, int *v_current_max, float v_speed, int v_min,
    ...
    bool value_changed = ImGui::DragInt("##min", v_current_min, v_speed, (v_min >= v_max) ? IM_INT_MIN : v_min,
                                        (v_min >= v_max) ? *v_current_max : ImMin(v_max, *v_current_max),
                                        display_format);
    ImGui::PopItemWidth();
    ImGui::SameLine(0, g.Style.ItemInnerSpacing.x);
    value_changed |= ImGui::DragInt("##max", v_current_max, v_speed,
                                    (v_min >= v_max) ? *v_current_min : ImMax(v_min, *v_current_min),
...
static bool InputTextFilterCharacter(unsigned int *p_char, ImGuiInputTextFlags flags, ImGuiTextEditCallback callback,
    ...
    if (flags &
        ...
        if (flags & ImGuiInputTextFlags_CharsDecimal) if (!(c >= '0' && c <= '9') && (c != '.') && (c != '-') &&
                                                          (c != '+') && (c != '*') && (c != '/'))
            return false;

        if (flags & ImGuiInputTextFlags_CharsHexadecimal) if (!(c >= '0' && c <= '9') && !(c >= 'a' && c <= 'f') &&
                                                              !(c >= 'A' && c <= 'F'))
            return false;

        if (flags & ImGuiInputTextFlags_CharsUppercase) if (c >= 'a' && c <= 'z')
            *p_char = (c += (unsigned int) ('A' - 'a'));

        if (flags & ImGuiInputTextFlags_CharsNoBlank) if (ImCharIsSpace(c))
            return false;
    }
    ...
    if (g.ActiveId == id || (edit_state.Id == id && is_multiline && g.ActiveId == draw_window->GetID("#SCROLLY"))) {
        ...
        if (cursor_is_visible)
            draw_window->DrawList->AddLine(cursor_screen_pos + ImVec2(0.0f, -g.FontSize + 0.5f),
                                           cursor_screen_pos + ImVec2(0.0f, -1.5f), GetColorU32(ImGuiCol_Text));
...
bool ImGui::Combo(const char *label, int *current_item, bool (*items_getter)(void *, int, const char **), void *data,
    ...
    RenderFrame(ImVec2(frame_bb.Max.x - arrow_size, frame_bb.Min.y), frame_bb.Max,
                GetColorU32(popup_opened || hovered ? ImGuiCol_ButtonHovered : ImGuiCol_Button), true,
...
bool ImGui::Selectable(const char *label, bool selected, ImGuiSelectableFlags flags, const ImVec2 &size_arg) {
    ...
    float max_x = (flags & ImGuiSelectableFlags_SpanAllColumns) ? ImGui::GetWindowContentRegionMax().x
...
bool ImGui::MenuItem(const char *label, const char *shortcut, bool selected, bool enabled) {
    ...
    bool pressed = ImGui::Selectable(label, false,
...
bool ImGui::BeginMenuBar() {
    ...
    PushClipRect(ImVec2(rect.Min.x + 0.5f, rect.Min.y - 0.5f + window->BorderSize),
...
bool ImGui::BeginMenu(const char *label, bool enabled) {
    ...
    bool menuset_opened = !(window->Flags & ImGuiWindowFlags_Popup) &&
    ...
    if (window->DC.LayoutType == ImGuiLayoutType_Horizontal) {
        ...
        pressed = ImGui::Selectable(label, opened, ImGuiSelectableFlags_Menu | ImGuiSelectableFlags_DontClosePopups |
                                                   (!enabled ? ImGuiSelectableFlags_Disabled : 0), ImVec2(w, 0.0f));
        ImGui::PopStyleVar();
        ImGui::SameLine();
        window->DC.CursorPos.x += (float) (int) (style.ItemSpacing.x * 0.5f);
    ...
    if (window->Flags & (ImGuiWindowFlags_Popup | ImGuiWindowFlags_ChildMenu)) {
        ...
        if (g.HoveredWindow == window && g.OpenedPopupStack.Size > g.CurrentPopupStack.Size &&
            ...
            if (ImGuiWindow *next_window = g.OpenedPopupStack[g.CurrentPopupStack.Size].Window) {
                ...
                tb.y = ta.y + ImMax((tb.y - extra) - ta.y,
                                    -100.0f);            // triangle is maximum 200 high to limit the slope and the bias toward large sub-menus
        ...
        want_close = (opened && !hovered && g.HoveredWindow == window && g.HoveredIdPreviousFrame != 0 &&
    ...
    else if (opened && pressed && menuset_opened) // menu-bar: click open menu to close
    {
        ...
        want_open = opened = false;
             ...
             (hovered && menuset_opened && !opened)) // menu-bar: first click to open, then hover to open others
        want_open = true;
    ...
    if (opened) {
        ...
        opened = BeginPopupEx(label,
                              flags); // opened can be 'false' when the popup is completely clipped (e.g. zero size display)
    }
...
bool ImGui::ColorEdit4(const char *label, float col[4], bool alpha) {
    ...
    if (ImGui::IsItemHovered())
        ImGui::SetTooltip("Color:\n(%.2f,%.2f,%.2f,%.2f)\n#%02X%02X%02X%02X", col[0], col[1], col[2], col[3],
                          IM_F32_TO_INT8(col[0]), IM_F32_TO_INT8(col[1]), IM_F32_TO_INT8(col[2]),
    ...
    if (label != label_display_end) {
        ImGui::SameLine(0, (window->DC.ColorEditMode == ImGuiColorEditMode_UserSelectShowButton) ? -1.0f
    ...
    if (window->DC.ColumnsCount > 1) {
        ...
        if (++window->DC.ColumnsCurrent < window->DC.ColumnsCount) {
            ...
            window->DC.ColumnsOffsetX =
                    ImGui::GetColumnOffset(window->DC.ColumnsCurrent) - window->DC.IndentX + g.Style.ItemSpacing.x;
    ...
    if (ImGui::Begin("ImGui Metrics", opened)) {
        ...
        struct Funcs {
            static void NodeDrawList(ImDrawList *draw_list, const char *label) {
                bool node_opened = ImGui::TreeNode(draw_list, "%s: '%s' %d vtx, %d indices, %d cmds", label,
                ...
                for (const ImDrawCmd *pcmd = draw_list->CmdBuffer.begin();
                    ...
                    bool draw_opened = ImGui::TreeNode((void *) (pcmd - draw_list->CmdBuffer.begin()),
                    ...
                    for (int i = elem_offset; i + 2 < elem_offset + (int) pcmd->ElemCount; i += 3) {
                        ...
                        for (int n = 0; n < 3; n++) {
                            ImDrawVert &v = draw_list->VtxBuffer[(draw_list->IdxBuffer.Size > 0)
                                                                 ? draw_list->IdxBuffer.Data[i + n] : i + n];
                        ...
                        if (ImGui::IsItemHovered())
                            overlay_draw_list->AddPolyline(triangles_pos, 3, ImColor(255, 255, 0), true, 1.0f,
        ...
        if (ImGui::TreeNode("Popups", "Opened Popups Stack (%d)", g.OpenedPopupStack.Size)) {
            for (int i = 0; i < g.OpenedPopupStack.Size; i++) {
                ...
                ImGui::BulletText("PopupID: %08x, Window: '%s'%s%s", g.OpenedPopupStack[i].PopupID,
